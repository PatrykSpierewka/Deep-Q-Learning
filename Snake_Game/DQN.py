...
class DQNAgent:
    ...
    def build_model(self):
        ...
        model.compile(loss='mse', optimizer=Adam(lr=0.00055))
        return model

    def remember(self, state, action, reward, next_state, done):
        self.memory.append((state, action, reward, next_state, done)) #save memory (action, reward)
...
class Evaluator:
    mode = False
    def __init__(self):
        self.score_hist = []                #history of results during the evaluation
        self.avg_score_hist = []            #history of all average scores
    ...
    def save_score(self):
        ...
        if not os.path.exists(path) or self.override == True:
            with open(path, 'w') as fp:         #save score to txt
...
ev = Evaluator()
train_score_hist = deque(maxlen=50)
